...
function renderInline(s, imgPrefix) {
  let t = escapeHtml(s);
  // 图片：images/ 开头的相对路径改写为 /media/<postId>/...
  t = t.replace(
    /!\[([^\]]*)\]\(([^)\s]+)(?:\s+&quot;[^&quot;]*&quot;)?\)/g,
    (m, alt, src) => {
      const finalSrc = imgPrefix && src.startsWith("images/") ? imgPrefix + src.slice(7) : src;
...
function renderMarkdown(src, imgPrefix) {
  const lines = String(src || "").replace(/\r\n/g, "\n").split("\n");
  const out = [];
  let i = 0;
  const inline = (s) => renderInline(s, imgPrefix);
