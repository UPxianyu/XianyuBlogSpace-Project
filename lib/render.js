...
function renderInline(s, imgPrefix, imgInfo) {
  ...
  // 保护已生成的 HTML 片段，避免后续自动链接误伤 img src / a href / code 内容
  const stash = [];
  const protect = (html) => {
    stash.push(html);
  ...
  const restore = (str) =>
    str.replace(/\u0000P(\d+)\u0000/g, (m, i) => stash[+i] || m);
  ...
  t = t.replace(
    ...
    (m, alt, src) => {
      ...
      if (imgPrefix && rawSrc.startsWith("images/")) {
        ...
        return protect(`<img src="${escapeHtml(imgPrefix + encodeURIComponent(webp))}" alt="${escapeHtml(rawAlt)}" loading="lazy"${dims}>`);
      }
      return protect(`<img src="${escapeHtml(rawSrc)}" alt="${escapeHtml(rawAlt)}" loading="lazy">`);
    }
  );
  t = t.replace(/`([^`]+)`/g, (m, c) => protect(`<code>${c}</code>`));
  ...
  t = t.replace(
    ...
    (m, text, href) => {
      ...
      return protect(`<a href="${escapeHtml(rawHref)}" target="_blank" rel="noopener noreferrer">${text}</a>`);
    }
  );
  // 自动链接：裸露的 http(s) URL 转为可点击链接（已保护的片段不受影响）
  t = t.replace(
    /(^|[\s>([【])(https?:\/\/[^\s<>"'）)\]】]+)/g,
    (m, lead, url) => {
      const u = url.replace(/[.,;:!?。，；：！？]+$/, "");
      if (!/^(https?:)/i.test(u)) return m;
      // u 已经过 escapeHtml，直接复用即可，避免 &amp; 二次转义
      return `${lead}<a href="${u}" target="_blank" rel="noopener noreferrer">${u}</a>`;
    }
  );
  return restore(t);
}
...
function renderMarkdown(src, imgPrefix, imgInfo) {
  ...
  while (i < lines.length) {
    ...
    let paraHtml = `<p>${inline(buf.join(" "))}</p>`;
    // 图注：段首是单张图片且后面带说明文字时，渲染为 figure + figcaption
    const fig = paraHtml.match(/^<p>(<img[^>]*>)\s*([\s\S]+?)<\/p>$/);
    if (fig && !/<img/.test(fig[2])) {
      const caption = fig[2].trim();
      if (caption) {
        paraHtml = `<figure>${fig[1]}<figcaption>${caption}</figcaption></figure>`;
      }
    }
    out.push(paraHtml);
  }
